...
<script lang="ts">
  ...
  function pos(idx: number): Array<number> {
    ...
    switch (idx) {
      ...
      case 1:
        return [xoff + 0.8 * off, yoff - 0.75 * hChar];
      case 2:
        return [xoff + off + 3, yoff];
      case 3:
        return [xoff + 0.8 * off, yoff + 0.75 * hChar];
      case 4:
        return [xoff + (title.length - itemWidth) / 2, yoff + hChar + 1.2];
      case 5:
        return [xoff + -0.8 * itemWidth * wChar, yoff + 0.75 * hChar];
      case 6:
        return [xoff + -itemWidth * wChar - 3, yoff];
      case 7:
        return [xoff + -0.8 * itemWidth * wChar, yoff - 0.75 * hChar];
      case 8:
        return [xoff + (title.length - itemWidth) / 2, yoff];
    }
  }
